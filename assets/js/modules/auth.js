...
export function initAuth() {
    ...
    if(DOM.modalCloseBtn) DOM.modalCloseBtn.addEventListener('click', () => DOM.loginModal.classList.remove('is-visible'));
    DOM.loginModal.addEventListener('click', (event) => {
        if (event.target === DOM.loginModal) DOM.loginModal.classList.remove('is-visible');
    });
    ...
    DOM.loginForm.addEventListener('submit', (event) => {
        ...
        if (usernameInput === FAKE_USER_CREDENTIALS.username && passwordInput === FAKE_USER_CREDENTIALS.password) {
            handleLoginSuccess();
            DOM.loginForm.reset();
            if(DOM.loginErrorMsg) DOM.loginErrorMsg.textContent = '';
        } else {
            if(DOM.loginErrorMsg) DOM.loginErrorMsg.textContent = 'Identifiant ou mot de passe incorrect.';
        }
    });
}